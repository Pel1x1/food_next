...
import { useRecipeStore } from "@/shared/providers/RecipeStoreProvider";
...
const RecipeClient: React.FC<Props> = ({ documentId }) => {
  const { cartStore, favouritesStore } = useStore();
  const recipeStore = useRecipeStore();

  useEffect(() => {
    if (!recipeStore.recipe && !recipeStore.loading && !recipeStore.error && documentId) {
  ...
  if (recipeStore.loading || (!recipeStore.error && !recipe)) {
    return (
  ...
  const isInCart = cartStore.items.some((item) => item.documentId === recipe.documentId);
        ...
        <div className={styles.recipe__heroInfoContainer}>
          ...
          <div className={styles.recipe__statsStrip}>
            <div className={styles.recipe__statItem}>
              <LuTimer size={24} />
              <Text view="title" color="accent" tag="h2" className={styles.recipe__statVal}>
...
export default observer(RecipeClient);
